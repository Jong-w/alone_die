...
class SVGA(nn.Module):
    ...
    def __init__(self, edge_index, num_nodes, num_features, num_classes, hidden_size=256, lamda=1,
        ...
        #self.encoding_time_1= nn.LSTM(input_size=100, hidden_size=1, num_layers=1, bidirectional=False, batch_first=True).to("cuda")
        ...
        #self.decoding_time = nn.LSTM(input_size=1, hidden_size=100, num_layers=1, bidirectional=False, batch_first=True).to("cuda")
        #self.decoding_time_1 = nn.LSTM(input_size=1, hidden_size=100, num_layers=1, bidirectional=False, batch_first=True).to("cuda")
    ...
    def forward(self, edge_index, for_loss=False):
        ...
        x = self.obs_features.reshape(self.obs_features.shape[2], self.obs_features.shape[0], self.obs_features.shape[1])

        #z1, _ =  self.encoding_time_1(x[:, :self.feature_dim])
        #z2, _ = self.encoding_time_2(x[:, self.feature_dim:])

        z0 = self.encoding_time(x)
